...
def get_boxes_data(image, boxes, labels):
    boxes_data=[]
    boxes_data_person_only=[]
    for box in boxes:
        xmin  = int((box.x - box.w/2) * image.shape[1])
        xmax  = int((box.x + box.w/2) * image.shape[1])
        ymin  = int((box.y - box.h/2) * image.shape[0])
        ...
        #            1e-3 * image.shape[0], 
        #            (0,255,0), 2)
        boxes_data.append([score, predicted_class, (xmin,ymin), (xmax,ymax)])
    
    boxes_data_person_only = [data for data in boxes_data if data[1] == "person"]
    top3_persons = sorted(boxes_data_person_only, key = lambda x: x[0], reverse = True)[0:3]
